...
def train_HSIC_IV(hsic_net, config, X, Y, Z):
...
def hsic_perm_test(X, Y, kernel_x, kernel_y, B=100, par=True):
    X, Y = to_torch(X).detach(), to_torch(Y).detach()
    hsic = HSIC(X, Y, kernel_x, kernel_y)
    ...
    def compute_stat(i, X, Y, kernel_x, kernel_y):
        rand = np.random.RandomState(i)
        n = Y.shape[0]
        # permute Y
        idx = rand.choice(n, n, replace=False)
        new_Y = Y[idx].clone()
        return HSIC(X, new_Y, kernel_x, kernel_y)

    hsic_perm = Parallel(n_jobs=n_job)(
        delayed(compute_stat(i, X, Y, kernel_x, kernel_y))(i=i) for i in range(B))

    hsic_perm = np.array(hsic_perm)
