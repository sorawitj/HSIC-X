...
def train_AR(X, Y, Z):
    ...
    coefs = AR.anchor_regression(df_X, df_Z, 2, 'Y').rx2('coeff')
    return coefs
...
def generate_data(n, int_par, causal_par):
    ...
    X1 = Z + U1 * (Z <= 3.8) + Z * rand.normal(size=n)
...
if __name__ == "__main__":
    ...
    melt_res_df = pd.read_csv("results/DG_compare_df.csv")

    sns.set(font_scale=1.4, style='white', palette=sns.set_palette("tab10"))

    g = sns.catplot(data=melt_res_df, kind="point",
                    x='IntStr', y='MSE', hue='Method', capsize=.07,
                    hue_order=['AR', 'OLS', 'Causal', 'HSIC-X-pen'],
                    markers=["o", "x", "d", "s"], linestyles=[':', '-', '--', '-.'],
                    aspect=1.3, height=3.5, ci=95)
    g._legend.remove()

    plt.legend(loc='upper center', bbox_to_anchor=(0.4, 1.14),
